...
public:
    ...
    //     : Person{ std::move(firstName), std::move(lastName), std::format("{}{}", firstName[0], lastName[0]) }
    // because then firstName and lastName could already have been moved into temporary
    // std::strings before firstName[O] and lastName[O] are used in the call to std::format().
    ...
    const std::string& getFirstName(this const Person& self)
    {
        return self.m_firstName;
    }
    std::string&& getFirstName(this Person&& self)
    {
    ...
    // Using C++23 explicit object parameters.
    const std::string& getLastName(this const Person& self)
    {
        return self.m_lastName;
    }
    std::string&& getLastName(this Person&& self)
    {
    ...
    // Using C++23 explicit object parameters.
    const std::string& getInitials(this const Person& self)
    {
        return self.m_initials;
    }
    std::string&& getInitials(this Person&& self)
    {
    ...
    // Only this single line of code is needed to add support for all six comparison operators.
